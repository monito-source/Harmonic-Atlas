jQuery(function ($) {
  const metaBox = $('.pd-colaborador-galeria-meta');
  if (!metaBox.length || typeof wp === 'undefined' || !wp.media) {
    return;
  }

  const input = metaBox.find('input[name="pd_colaborador_galeria"]');
  const preview = metaBox.find('.pd-colaborador-galeria-preview');
  const selectBtn = metaBox.find('.pd-colaborador-galeria-select');
  const clearBtn = metaBox.find('.pd-colaborador-galeria-clear');
