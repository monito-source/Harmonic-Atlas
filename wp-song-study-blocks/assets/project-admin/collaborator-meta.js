jQuery(function ($) {
  const metaBox = $('.wpssb-collaborator-gallery-meta');
  if (!metaBox.length || typeof wp === 'undefined' || !wp.media) {
    return;
  }

  const input = metaBox.find('input[name="pd_colaborador_galeria"]');
  const preview = metaBox.find('.wpssb-collaborator-gallery-preview');
  const selectBtn = metaBox.find('.wpssb-collaborator-gallery-select');
  const clearBtn = metaBox.find('.wpssb-collaborator-gallery-clear');
