jQuery(function ($) {
  const metaBox = $('.wpssb-project-gallery-meta');
  if (!metaBox.length || typeof wp === 'undefined' || !wp.media) {
    return;
  }

  const input = metaBox.find('input[name="pd_proyecto_galeria"]');
  const preview = metaBox.find('.wpssb-project-gallery-preview');
  const selectBtn = metaBox.find('.wpssb-project-gallery-select');
  const clearBtn = metaBox.find('.wpssb-project-gallery-clear');
